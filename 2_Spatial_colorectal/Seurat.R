...
data <- Read10X_h5("Parent_Visium_Human_ColorectalCancer_filtered_feature_bc_matrix.h5")
...
FeaturePlot(data1,features = c("SPP1"),label = T,ncol = 1)
VlnPlot(data1,features = c("CDH1"),pt.size = 0.1,ncol=1)

################Cell annotation###################
Idents(data1) <- "SCT_snn_res.1"
cluster_name <- plyr::mapvalues(x = data1@active.ident, from = 0:17, to = c("clu0__Cancer cell","clu1__Macrophage","clu2__Cancer cell","clu3__Fibroblast","clu4__Plasma cell","clu5__Cancer cell",
                                                                            "clu6__Cancer cell","clu7__Cancer cell","clu8__Cancer cell","clu9__Smooth muscle cell","clu10__Cancer cell","Clu11__Cancer cell",
                                                                            "Clu12__Cancer cell","Clu13__Cancer cell","clu14__Fibroblast","Clu15__Cancer cell","Clu16__Cancer cell","clu17__Fibroblast"))
...
cluster_name_slim <- factor(unlist(lapply(strsplit(as.character(cluster_name), "__"),function(input){input[2]})),
                            levels = c("Macrophage","Cancer cell","Fibroblast","Plasma cell","Smooth muscle cell"))
...
markers <- c("SIGLEC1","CD22","SIGLEC15","CD33","SIGLEC5","SIGLEC7","SIGLEC8","SIGLEC9","SIGLEC10","SIGLEC11","SIGLEC16")
